...
class External extends CI_Controller {
    ...
    public function get_vendor() {
      if ($_GET) {
        ...
        if(isset($menu['search']) && isset($menu['role']) && !isset($menu['roleid'])){
            if($menu['type']=='add')
          $data = $this->common_model->get_vendor_by_name($menu['search'], $menu['role']);
          else
           $data = $this->common_model->get_vendor_by_name($menu['search'], 0);
        }
        else if(isset($menu['roleid']))
        {
            $priority=$this->common_model->priority($menu['roleid']);
            $data=$this->common_model->get_vendor_by_name_by_prority('',$menu['role'],$priority[0]['priority']);
        }
        else {
     ...
     public function auto_vendor() {
      if ($_GET) {
        $menu=$this->security->xss_clean($_GET);
        if(isset($menu['search']) && isset($menu['role']) && !isset($menu['vendor'])){
           $priority=$this->common_model->priority($menu['role']);
           $data=$this->common_model->get_vendor_by_name_by_prority1($menu['search'],$menu['vendor'],$menu['role'],$priority[0]['priority']);
        }
        else
        {
           $priority=$this->common_model->priority($menu['role']);
           $data=$this->common_model->get_vendor_by_name_by_prority1('',$menu['vendor'],$menu['role'],$priority[0]['priority']); 
        }
        echo json_encode($data);
      }
    }
    ...
    if ($_GET) {
      $subject = $this->security->xss_clean($_GET);
      $data = $this->common_model->exists('user', ['phone' => $subject['search'],'delete_user'=>0]);
      if ($data) {
        $responce = [
          'error' => 1,
