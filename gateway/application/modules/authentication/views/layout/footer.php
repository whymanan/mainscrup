...
$(document).ready(function() {
    ...
    //   console.log(id);
      if (id != "") {
        ...
        clearTimeout(timerId);
                 ...
                 if(timeLeft==0)
                 {
                $('#resend').css("display","initial"); 
                $('#timer').text('');
                 $.ajax({
                   url: '<?php echo base_url('auth/remove_otp'); ?>/'+data.user_id,
                   type: 'GET',
                   dataType:'json',
                   });
                 }
    ...
    $(document).on("click", "#resend", function() {
      ...
      if (id != "") {
        $.ajax({
          url: '<?php echo base_url('auth/resend_otp'); ?>',
          type: 'POST',
          dataType:'json',
          data: {
            id: id,
        ...
        clearTimeout(timerId);
                ...
                $('#timer').text('');
                 $.ajax({
                   url: '<?php echo base_url('auth/remove_otp'); ?>/'+id,
                   type: 'GET',
                   dataType:'json',
                   });
                 }
