        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col-8">
                        <h3 class="mb-0">Legal Sevices Commission</h3>
                    </div>
...
<script>
  ...
  function Delete(id) {
    ...
    if (sureDel == true) {
      $.ajax({
        type: "GET",
        url: "<?php echo base_url('recharge/RechargeController/m_commissiondelete/')?>" + id,
...
var $addCommissionForm = $('#addCommissionForm');
    ...
    if (sureDel == true) {
      $.ajax({
        type: "GET",
        url: "<?php echo base_url('legalservice/LegalController/commissionedit/') ?>" + id,
          ...
          if (response != "") {
            $('#role_id').val(role_id);          
                $('#start').val(response.start_range);
                  $( "#service option:selected" ).text(response.service);
        ...
        var role_id = $('#role').val();
            $.ajax({
              url: '<?php echo base_url('legalservice/LegalController/commissionaddupdate'); ?>',
              type: 'POST',
              data: {"addupdate": role_id,'service':response.service_id,"<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"},
             beforeSend: function(){
