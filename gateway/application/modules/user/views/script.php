...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('input[name="phone_no1"]').on('change', function() {
      ...
      $.ajax({
        ...
        beforeSend: function() {
          $('input[name="phone_no1"]').parent().find('label').append('<span><img width="24" height="24" src="<?php echo base_url('optimum/loading.svg') ?>" /></span>');
        },
        success: function(data) {
          var result = JSON.parse(data);
          if (result.error) {
            $('input[name="phone_no1"]').css({
            ...
            $('input[name="phone_no1"]').parent().find('label').css({
              "color": "#525f7f",
            }).html('<label class="form-control-label">Mobile</label>');
      ...
      else
      {
            $('input[name="phone_no1"]').css({
              ...
              "color": "#ff5050",
            }).html('Please vailid mobile number');
          $submit.attr("disabled", "disabled");
...
console.log("<?php echo $this->session->userdata('user_roles')?>");
<script>
...
//        );
   ...
   const generatePDF = async(name,id) => {
       const { PDFDocument, rgb, degrees } = PDFLib;
       var pdffile;
         pdffile="<?php echo base_url('optimum/BC_CERTIFICATE.pdf')?>";
       ...
       if(pancard!='')
       {
        ...
        if(shopaddress!='')
       {
        let result = shopaddress.length
         if(result>=55)
         {
           text1=shopaddress.substr(0,55);
           text2=shopaddress.substr(55,result);
         ...
         {
           firstPage.drawText(shopaddress, {
           x: 150,
           y: 253,
           size: 20,
           font: SanChezFont,
