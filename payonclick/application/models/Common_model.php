...
function get_admin_wallet(){
  ...
  public function submenu_exists($menu, $parent)
  {
    ...
    function update($action,$field, $id, $table){
        ...
        // print_r($action);exit;
        if($this->db->where($field,$id)->update($table,$action))
        {
            return true;
        }
        //  print_r($this->db->last_query());
...
function get_wallet_by_id($id,$field){
  ...
  public function get_wallet_bal(){
    ...
    public function getBeneficiaryMobile($bid) {
      $result = $this->db->select('beneficiary_mobile')->get_where('beneficiary_list', array('beneficiaryId' => $bid));
      if ($result->num_rows()) {
        return $result->row()->beneficiary_mobile;
      } else {
        return false;
      }
    }
